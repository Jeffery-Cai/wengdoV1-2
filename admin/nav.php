...
{
// 删除函数
	delMore("del","wd_nav","nav_id","./nav.php");
}
$datacount=selectDb("SELECT * FROM wd_nav");
...
// 查询
 $data=selectDb("SELECT * FROM wd_nav ORDER BY nav_id DESC LIMIT {$offset},{$per_record}");
?>
<!doctype html>
<html lang="zh-ch">
	<!-- container-fluid -->
    ...
    <!-- 中间内容 -->
		<div id="content">
			...
			<div id="breadcrumb">
				...
				<a href=""><i class="icon-th-list"></i>顶级导航管理</a>
				<a href="article.php" class="current"><i class="icon-th-list"></i> 所有导航</a>
			</div>
									...
									<tbody role="alert" aria-live="polite" aria-relevant="all">
										...
										<tr>
											<td><div class="checker" id="uniform-undefined"><span>
												<input type="checkbox" style="opacity: 0;" name="del[]" value="<?php echo $value["nav_id"] ?>"></span></div></td>
											<td><?php echo $value["nav_id"] ?></td>
											<td><?php echo $value["nav_name"] ?></td>
											<td><?php echo $value["nav_links"] ?></td>

											<td><?php echo $value["nav_pid"] ?></td>
											<td><a href="nav_add_edit.php?id=<?php echo $value["nav_id"] ?>" class="tip-top" data-original-title="编辑"><i class="icon-pencil"></i></a></td>
										</tr>
										<?php }?>
									</tbody>
								</table>
